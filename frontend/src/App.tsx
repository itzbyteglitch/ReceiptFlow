...
function Details({record,onBack,onDelete,onSave}:{record:ReceiptRecord;onBack:()=>void;onDelete:()=>void;onSave:(updated:ReceiptRecord)=>void}) {
  ...
  const [form,setForm]=useState({merchant:{...record.merchant},receipt:{...record.receipt},customer:{...record.customer},payment:{...record.payment},metadata:{...record.metadata},items:record.items.map((item)=>({...item}))});
  ...
  return <main className="app-shell"><header className="topbar"><button className="back" onClick={onBack}><ArrowLeft size={18}/> Dashboard</button><div className="detail-actions"><button className="secondary-button" onClick={()=>setEditing(true)}>Edit details</button><button className="danger" onClick={onDelete}><Trash2 size={17}/> Delete receipt</button></div></header>
    ...
    <section className="panel"><div className="section-head"><div><h2>Items</h2><span>{record.items.length} items</span></div></div><div className="items-table"><div className="item-head"><span>Item</span><span>Product type / category</span><span>Qty</span><span>Total</span></div>{record.items.map(i=><div className="item-row" key={i.id}><span><strong>{i.name}</strong><small>{i.description}</small></span><span>{i.category}</span><span>{i.quantity} {i.unit}</span><strong>{money(i.total_price,record.receipt.currency)}</strong></div>)}</div></section>
    {editing&&<div className="edit-modal-backdrop"><section className="edit-modal"><div className="upload-modal-head"><div><p className="eyebrow">EDIT RECEIPT</p><h2>Receipt details</h2><small>Prices, totals, taxes and item amounts are locked.</small></div><button className="icon-button" onClick={()=>setEditing(false)}><X size={18}/></button></div><div className="edit-grid">
      ...
      <div className="edit-wide item-edit-section"><div className="edit-section-title"><div><p className="eyebrow">ITEM CLASSIFICATION</p><strong>Product types / categories</strong><small>Edit each item independently. Prices and quantities are locked.</small></div><span className="locked-badge">Financial fields locked</span></div><div className="edit-item-list">{form.items.map((item:any,index:number)=><label className="edit-item-card" key={item.id}><span className="edit-item-number">{String(index+1).padStart(2,"0")}</span><span className="edit-item-copy"><strong>{item.name}</strong><small>{item.quantity} {item.unit} · {money(item.total_price,record.receipt.currency)}</small></span><span className="category-input-wrap"><input aria-label={`Category for ${item.name}`} value={item.category} onChange={e=>setForm((f:any)=>({...f,items:f.items.map((x:any,i:number)=>i===index?{...x,category:e.target.value}:x)}))} /></span></label>)}</div></div>
    ...
    </div><div className="edit-footer"><button className="upload-cancel" onClick={()=>setEditing(false)}>Cancel</button><button className="login-button save-button" onClick={save} disabled={saving}><Save size={16}/>{saving?"Saving…":"Save changes"}</button></div></section></div>}
